...
export default function App() {
  ...
  const handleClaimVoucher = async (e) => {
    ...
    try {
      const voucherRef = doc(db, 'vouchers', codeClean);
      ...
      if (rewardType === 'tickets') {
        ...
        await updateDoc(userRef, { balance: increment(rewardAmount) });
      }

      showNotification(`Sukses! Berhasil klaim ${rewardAmount} ${rewardType === 'tickets' ? 'Tiket' : 'Koin'}!`, "success");
      setVoucherInput('');
    } catch (err) {
      console.error(err);
        ...
        {currentView === 'wallet' ? (
          <main className="max-w-md mx-auto px-4 mt-8 flex flex-col gap-4">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-lg">
              ...
              <form onSubmit={handleClaimVoucher} className="flex gap-2 mt-3">
                <input 
                  type="text" 
                  placeholder="Masukkan Kode Voucher" 
                  value={voucherInput}
                  onChange={(e) => setVoucherInput(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 uppercase tracking-wider"
                />
                <button 
                  type="submit" 
                  disabled={voucherLoading}
                  className="bg-blue-600 hover:bg-blue-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-colors cursor-pointer disabled:bg-slate-700 shrink-0"
                >
