...
export default function App() {
  ...
  // Navigasi Halaman ('game' atau 'wallet')
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-slate-950 text-slate-100 font-sans pb-10">
      ...
      {/* NAVBAR ATAS (DIPERBAIKI AGAR TOMBOLNYA MUDAH DIKLIK) */}
      <nav className="bg-slate-900 border-b border-slate-800 sticky top-0 z-50 px-4 py-3 flex justify-between items-center shadow-md">
        ...
        <div className="flex items-center gap-2">
           {/* Tombol Menu Dompet (Koin & Tiket digabung jadi satu tombol interaktif agar mudah disentuh) */}
           <button 
      ...
      {/* TAMPILAN HALAMAN DOMPET ATAU GAME */}
      {currentView === 'wallet' ? (
        
        /* --- HALAMAN DOMPET --- */
        <main className="max-w-md mx-auto px-4 mt-8 flex flex-col gap-4">
          <div className="flex items-center gap-4 mb-2">
            <button onClick={() => setCurrentView('game')} className="p-2.5 bg-slate-800 hover:bg-slate-700 rounded-xl text-slate-300 flex items-center gap-2 text-sm font-bold">
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        // Pastikan totalSpins ada (jika user lama, set 0)
        setUserData({ ...data, totalSpins: data.totalSpins || 0 });
      } else {
        handleLogout();
      }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        // Daftar baru (tambahkan totalSpins = 0)
        await setDoc(docRef, { username: formUsername.toLowerCase(), password: formPassword, balance: 0, tickets: 0, totalSpins: 0, joinedAt: serverTimestamp() });
        localStorage.setItem('faucet_session', formUsername.toLowerCase());
        setCurrentUser(formUsername.toLowerCase());
      }
  ...
  const startAd = () => { 
    // GANTI TULISAN DI BAWAH INI DENGAN DIRECT LINK IKLAN ADSTERRA ANDA
    // window.open("LINK_DARI_ADSTERRA_PASTE_DISINI", "_blank");
  ...
  const handleSpin = async () => {
    ...
    try { 
      // Tiket -1, dan Total Spin +1 setiap kali main
      await updateDoc(docRef, { tickets: increment(-1), totalSpins: increment(1) }); 
    } catch (err) { setIsSpinning(false); return; }
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-slate-950 text-slate-100 font-sans pb-10">
      {/* NAVBAR ATAS */}
      <nav className="bg-slate-900 border-b border-slate-800 sticky top-0 z-40 p-4 flex justify-between items-center">
        <h1 className="font-bold text-xl flex items-center gap-2 cursor-pointer" onClick={() => setCurrentView('game')}>
          <Trophy className="text-yellow-500" size={24}/> 
          <span className="hidden sm:block">Faucet<span className="text-blue-500">Spin</span></span>
        </h1>
        <div className="flex gap-2 text-sm items-center">
           {/* Tombol Balance (Bisa di klik untuk ke menu dompet) */}
           <button 
              onClick={() => setCurrentView('wallet')}
              className="bg-slate-800 hover:bg-slate-700 px-3 py-1.5 rounded-full border border-slate-700 flex items-center gap-2 transition-colors"
           >
              <Coins size={16} className="text-yellow-400"/> 
              <span className="font-semibold">{userData.balance}</span>
           </button>
           {/* Tombol Tiket (Bisa di klik) */}
           <button 
              onClick={() => setCurrentView('wallet')}
              className="bg-slate-800 hover:bg-slate-700 px-3 py-1.5 rounded-full border border-slate-700 flex items-center gap-2 transition-colors"
           >
              <Ticket size={16} className="text-green-400"/> 
              <span className="font-semibold">{userData.tickets}</span>
           </button>
           <button onClick={handleLogout} className="text-red-400 p-2 hover:bg-slate-800 rounded-full transition-colors"><LogOut size={18}/></button>
        </div>
      </nav>

      {/* RENDER HALAMAN BERDASARKAN STATE currentView */}
      {currentView === 'wallet' ? (
        
        /* --- HALAMAN DOMPET (WALLET) --- */
        <main className="max-w-md mx-auto px-4 mt-8 flex flex-col gap-4 animate-[scale-in_0.2s_ease-out]">
          <div className="flex items-center gap-4 mb-4">
            <button onClick={() => setCurrentView('game')} className="p-2 bg-slate-800 rounded-full text-slate-300 hover:text-white">
              <ArrowLeft size={20} />
            </button>
            <h2 className="text-2xl font-bold flex items-center gap-2"><Wallet className="text-blue-500" /> Dompet Member</h2>
          </div>

          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <p className="text-slate-400 text-sm mb-1 uppercase font-bold">Total Koin</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
              <p className="text-slate-400 text-xs mb-2 uppercase font-bold">Tiket Tersedia</p>
              <div className="flex items-center gap-2">
                <Ticket size={24} className="text-green-400" />
                <span className="text-2xl font-bold">{userData.tickets}</span>
              </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-2xl font-bold">{userData.totalSpins}</span>
              </div>
            </div>
          </div>

          <div className="mt-8 bg-blue-600/10 border border-blue-500/30 p-4 rounded-xl text-center">
            <p className="text-blue-300 text-sm">Mainkan terus untuk mengumpulkan lebih banyak koin! Fitur penarikan (withdraw) akan segera hadir.</p>
          </div>
        </main>

      ) : (

        /* --- HALAMAN GAME (RODA PUTAR) --- */
        <main className="max-w-md mx-auto px-4 mt-8 flex flex-col items-center gap-8 animate-[scale-in_0.2s_ease-out]">
          <button onClick={startAd} disabled={isSpinning || showAd} className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 p-4 rounded-xl font-bold transition-colors">
            <Video/> Tonton Iklan (+1 Tiket)
          </button>
          ...
          <button onClick={handleSpin} disabled={isSpinning || userData.tickets <= 0} className="w-full bg-gradient-to-b from-yellow-400 to-yellow-600 text-black p-4 rounded-full font-black text-lg disabled:from-slate-700 disabled:to-slate-800 disabled:text-slate-500 transition-all shadow-[0_4px_0_#a16207] active:shadow-none active:translate-y-[4px] disabled:shadow-none disabled:translate-y-0">
            {isSpinning ? "MEMUTAR..." : "SPIN SEKARANG"}
          </button>
        </main>
      )}
      ...
      {wonPrize !== null && !isSpinning && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-50 text-center">
          <div className="bg-slate-900 p-8 rounded-2xl w-full max-w-sm border-2 border-yellow-500 animate-[scale-in_0.3s_ease-out]">
            <h2 className="text-2xl mb-4">Selamat!</h2><div className="text-5xl font-black text-yellow-400 mb-6 drop-shadow-lg">+{wonPrize}</div>
            <button onClick={() => setWonPrize(null)} className="bg-slate-800 hover:bg-slate-700 p-3 rounded-xl w-full font-bold">Tutup & Lanjutkan</button>
          </div>
        </div>
      )}
      {notification && <div className="fixed bottom-4 right-4 bg-slate-800 p-4 rounded-lg z-50 shadow-xl border border-slate-700 font-medium animate-bounce">{notification.msg}</div>}

      <style dangerouslySetInnerHTML={{__html: `@keyframes scale-in { 0% { transform: scale(0.9); opacity: 0; } 100% { transform: scale(1); opacity: 1; } }`}} />
    </div>
  );
}
  const [authLoading, setAuthLoading] = useState(false);
